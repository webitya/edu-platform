...
const HomeHeroSection = () => {
  return (
    <div className="relative w-full h-[260px] sm:h-[90vh] font-sans bg-black md:mb-[30px] mb-[410px]">
      {/* Carousel */}
      <Carousel
        autoPlay
        infiniteLoop
      ...
      >
        <div>
          <img
            ...
            className="object-cover h-[260px] sm:h-[90vh] w-full"
          />
        </div>
        <div>
          <img
      ...
      <div className="absolute inset-0 flex flex-col items-start justify-center text-white px-6 md:px-20 z-10">
        ...
        <h1 className="md:text-6xl text-2xl font-extrabold leading-tight md:mt-2 mt-1">
          LAW LEARNING BENCH
        </h1>
        <p className="md:mt-4 mt-2 max-w-xl text-base md:text-lg">
          India’s Premier Advocacy Skill Development Platform — transforming legal knowledge into courtroom expertise.
        </p>
        <div className="flex gap-4 md:mt-6 mt-3">
          <button className="bg-yellow-500 hover:bg-yellow-600 text-black font-bold px-4 md:px-6 md:py-2 py-2 rounded shadow">
            JOIN WITH US
          </button>
          <button className="bg-white hover:bg-gray-100 text-yellow-600 font-bold px-4 md:px-6 md:py-2 py-2 rounded shadow">
            OUR PROGRAMS
          </button>
        </div>
      </div>

      {/* Bottom Yellow Info Cards (Overlay) */}
      <div className="absolute mt-[10px] sm:mt-[-110px] left-1/2 transform -translate-x-1/2 w-[90vw] md:w-[70vw] grid grid-cols-1 sm:grid-cols-3 bg-yellow-500 text-white z-20 rounded-md overflow-hidden shadow-lg">
